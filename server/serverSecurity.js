...
if (!process.env.MAX_INIT_DATA_AGE_SECONDS) process.env.MAX_INIT_DATA_AGE_SECONDS = '86400';
...
const allowedOrigins = new Set(rawAllowedOrigins.split(',').map((origin) => origin.trim()).filter(Boolean));
...
export function createRestrictedCors(options = {}) {
  return cors({
    ...options,
...
export function createGuardedJson(options = {}) {
  const jsonMiddleware = express.json(options);

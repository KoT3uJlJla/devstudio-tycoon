...
function patchFile(fileName, patcher) {
  ...
  try {
    ...
    console.warn(`referrals-hardening: cannot read ${fileName}`, error?.message || error);
    return;
  }
  ...
  try {
    ...
    console.warn(`referrals-hardening: cannot write ${fileName}`, error?.message || error);
  }
}
...
const referralHelpers = String.raw`
...
function referrerIdFromStartParam(startParam) {
  ...
  const match = compact.match(/^r_([a-z0-9]{2,16})$/i);
  if (!match) return "";
  const mixed = Number.parseInt(match[1], 36);
  if (!Number.isFinite(mixed)) return "";
  const decoded = (mixed ^ REFERRAL_CODE_MASK) >>> 0;
  return decoded > 0 ? String(decoded) : "";
}
...
const referralAsyncHelpers = String.raw`
async function ensureReferralForUser(telegramUser) {
  ...
  const secondLevelReferrerTelegramId = parentReferral?.referrerTelegramId && parentReferral.referrerTelegramId !== invitedTelegramId && parentReferral.referrerTelegramId !== referrerTelegramId
...
async function incrementReferralCounter(telegramId, field, invitedTelegramId, score, tier) {
  ...
  await patchEconomy(id, {
    $inc: { [field]: 1 },
    $push: {
      ledger: {
        $each: [buildLedgerEntry("referral_qualified", 0, `referral:${tier}`, { invitedTelegramId, qualifyingScore: score })],
        $slice: -80,
      },
    },
  });
...
async function qualifyReferralIfEligible(telegramUser, saveData, meta = {}) {
  ...
  const qualified = await db.collection("referrals").findOneAndUpdate(
  ...
  if (referral.secondLevelReferrerTelegramId) {
    await incrementReferralCounter(referral.secondLevelReferrerTelegramId, "qualifiedSecondLevelReferrals", invitedTelegramId, score, "second");
  }
...
function patchIndexJs(source) {
  let next = source;
  if (!next.includes("REFERRAL_QUALIFY_MIN_SCORE")) {
    next = next.replace("function publicEconomy(economy) {", `${referralHelpers}\nfunction publicEconomy(economy) {`);
  }
  if (!next.includes("referralStartParam:")) {
    next = next.replace("lastRating: economy?.lastRating || null,", "lastRating: economy?.lastRating || null,\n    referralStartParam: referralStartParamFromTelegramId(economy?.telegramId),");
  }
  if (!next.includes("async function ensureReferralForUser")) {
    next = next.replace("async function grantStars(economy, amount, reason, meta = {}) {", `${referralAsyncHelpers}\nasync function grantStars(economy, amount, reason, meta = {}) {`);
  }
  ...
  if (!next.includes('db.collection("referrals").createIndex({ invitedTelegramId: 1 }')) {
    next = next.replace(
  ...
  if (!next.includes('qualifyReferralIfEligible(req.telegramUser, nextData')) {
    next = next.replace(
...
const commissionHelpers = String.raw`
...
async function ensureReferralEconomyForCommission(deps, telegramId) {
  const id = String(telegramId || "");
  if (!id) return null;
...
async function grantReferralPurchaseCommissions(deps, invoice) {
  ...
  const tiers = [
    { tier: "direct", telegramId: referral.referrerTelegramId, amount: referralCommissionAmount(invoice.amountStars, Number(process.env.REFERRAL_DIRECT_PURCHASE_SHARE || 0.10)) },
    { tier: "second", telegramId: referral.secondLevelReferrerTelegramId, amount: referralCommissionAmount(invoice.amountStars, Number(process.env.REFERRAL_SECOND_PURCHASE_SHARE || 0.03)) },
  ].filter((item) => item.telegramId && item.amount > 0);

  for (const item of tiers) {
    const marker = { invoiceId: invoice.invoiceId, tier: item.tier };
    try {
    ...
    await deps.patchEconomy(String(item.telegramId), {
      ...
      $push: {
        ledger: {
          $each: [{ id: crypto.randomUUID(), kind: "referral_purchase", amount: item.amount, reason: `referral:${item.tier}:purchase`, meta: { invoiceId: invoice.invoiceId, invitedTelegramId: invoice.telegramId, itemId: invoice.itemId, sourceAmountStars: invoice.amountStars }, createdAt: new Date() }],
          $slice: -80,
        },
      },
    });
...
function patchStarsPayments(source) {
  let next = source;
  if (!next.includes("function referralCommissionAmount")) {
    next = next.replace("async function applyPaidInvoice(deps, invoice, payment) {", `${commissionHelpers}\nasync function applyPaidInvoice(deps, invoice, payment) {`);
  }
  if (!next.includes("await grantReferralPurchaseCommissions(deps, invoice);")) {
    next = next.replace(
  ...
  requirePatch(next, "function referralCommissionAmount", "stars referral commission helpers");
  requirePatch(next, "grantReferralPurchaseCommissions(deps, invoice)", "stars referral commission call");
  return next;
}

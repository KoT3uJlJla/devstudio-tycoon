...
function duration(project){ return project.isTutorial?30:Math.round(n(130+(GENRES[project.genre]||1)*52+(PLATFORMS[project.platform]?.t||1)*28,120,420)); }
function cost(project,data){
  ...
  const raw=360+(duration(project)/10)*28+(GENRES[project.genre]||1)*260+(PLATFORMS[project.platform]?.t||1)*280+tech*95;
...
function devQueue(isTutorial){ return isTutorial?[]:[{instanceId:id(),scenarioId:"team-burnout",progressAt:32,triggered:false},{instanceId:id(),scenarioId:"ui-contrast",progressAt:66,triggered:false}]; }
...
export function startDevelopmentAction(saveData,draft){
  ...
  const durationSeconds=duration(p), devCost=cost({...p,durationSeconds},data);
  if((Number(data.coins)||0)-devCost<MIN_COINS) throw actionError("not_enough_coins",402);
  const now=Date.now();
  return normalize({...data,coins:(Number(data.coins)||0)-devCost,screen:"develop",tutorialStep:data.tutorialDone?data.tutorialStep:4,selectedProject:{...p,durationSeconds,devCost,techComplexity:PLATFORMS[platform].t+(has(data,"engine-v2")?.25:0)+(has(data,"ai-assist")?.18:0),startedAt:now,progress:Math.max(1,Number(p.progress)||0),promotionUsed:false,promotionBoost:0,devGlitchTriggered:false,devEventQueue:devQueue(p.isTutorial),pendingDevEvent:null,devDecisionScoreBonus:0,devDecisionSalesMultiplier:1,devDecisionLog:[],serverActionAt:now},lastLedger:devCost>0?ledger(data,`Старт разработки: ${clean(p.name)}`,-devCost,"expense"):data.lastLedger,lastSavedAt:now});
}

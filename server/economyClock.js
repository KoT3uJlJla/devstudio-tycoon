...
function safeInt(value, min = 0, max = Number.MAX_SAFE_INTEGER) { return Math.floor(clampNumber(value, min, max)); }
function safeArray(value) { return Array.isArray(value) ? value : []; }
function safeText(value, fallback = "") { return String(value || fallback).replace(/[<>"'`]/g, "").replace(/\s+/g, " ").trim().slice(0, 100) || fallback; }
function researchHas(data, id) { return safeArray(data?.unlockedResearchIds).includes(id); }
...
function sanitizeLedger(ledger) {
  return safeArray(ledger).filter(isPlainObject).map((entry) => ({
...
function sanitizeActiveGames(games) {
  return safeArray(games).filter(isPlainObject).map((game) => ({
...
function sanitizeMarketEvents(events) {
  return safeArray(events).filter(isPlainObject).map((event) => ({
    ...event,
...
function updateServerAudience(state, newMonth) {
  ...
  return { mood, desiredGenreId: genreTuple[0], desiredThemeId: themeTuple[0], vibe, lastUpdatedMonth: newMonth, revealedUntilMonth: safeInt(previousAudience.revealedUntilMonth, -1, 999999), learnedFrom };
}
...
function advanceServerEconomyOneDay(state) {
  ...
  let activeMarketEvents = sanitizeMarketEvents(state.activeMarketEvents).map((event) => ({ ...event, daysRemaining: safeInt(event.daysRemaining, 0, 31) - 1 })).filter((event) => event.daysRemaining > 0);
  ...
  if (hasEventSlot && marketMustRecover && eventChance < 0.032) {
    const event = makeServerMarketEvent(day, "positive"); activeMarketEvents = [event, ...activeMarketEvents]; newsFeed = [newsFromServerMarketEvent(event), ...newsFeed].slice(0, 8); ledger.push(serverLedgerEntry(day, "Глобальное событие: " + event.title, 0, "event")); marketMustRecover = false;
  } else if (hasEventSlot && !marketMustRecover && eventChance < 0.018) {
    const event = makeServerMarketEvent(day, "positive"); activeMarketEvents = [event, ...activeMarketEvents]; newsFeed = [newsFromServerMarketEvent(event), ...newsFeed].slice(0, 8); ledger.push(serverLedgerEntry(day, "Глобальное событие: " + event.title, 0, "event"));
  } else if (hasEventSlot && !marketMustRecover && eventChance > 0.982) {
    const event = makeServerMarketEvent(day, "negative"); activeMarketEvents = [event, ...activeMarketEvents]; newsFeed = [newsFromServerMarketEvent(event), ...newsFeed].slice(0, 8); ledger.push(serverLedgerEntry(day, "Глобальное событие: " + event.title, 0, "event")); marketMustRecover = true;
  }
  ...
  activeGames = activeGames.map((game) => {
    ...
    let eventBoost = 0;
    if (eventRoll < 0.065 + liveOps && game.score >= 7) { eventBoost = 0.2; eventText = "Стримеры подняли хайп: популярность выросла."; }
    else if (eventRoll > 0.9 && !researchHas(state, "producer-calendar")) { eventBoost = -0.22; eventText = "Комьюнити устало: популярность просела."; }
    else if (eventRoll > 0.84 && game.score >= 8.8) { eventBoost = 0.14; eventText = "Фан-арты пошли в чаты: плюс органика."; }
    ...
    return { ...game, popularity, totalEarned: safeInt(game.totalEarned, 0, Number.MAX_SAFE_INTEGER) + earned, lifeDaysRemaining: Math.max(0, safeInt(game.lifeDaysRemaining, 0, 30) - 1), lastEvent: eventText };
  ...
  let next = { ...state, coins, gameDay: day, activeGames, activeMarketEvents, marketMustRecover, newsFeed, audience, dailyPassiveIncome, weeklyExpenseTotal, lastLedger: ledger.slice(-10) };
  if (newMonth !== oldMonth || next.coins < 0) next = updateServerBankruptcyState(next, newMonth, oldMonth, [...next.lastLedger]);
  return { ...next, lastLedger: safeArray(next.lastLedger).slice(-10), newsFeed: safeArray(next.newsFeed).slice(0, 8) };
}

export function advanceServerEconomy(data) {
  ...
  let next = data;
  for (let index = 0; index < days; index += 1) next = advanceServerEconomyOneDay(next);
  return { ...next, lastGameTickAt: lastGameTickAt + days * ECONOMY_GAME_DAY_MS, lastSavedAt: Date.now() };
}

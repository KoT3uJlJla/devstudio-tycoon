...
function patchServerIndexShopItems() {
  ...
  if (
    source.includes('coins_100k')
    && source.includes('costStars: 79')
    && source.includes('product_instinct: { title: "Продуктовое чутьё", costStars: 199')
    && !source.includes('coins_small')
    && !source.includes('coins_medium')
  ) return;
  const shopItems = [
    'const SHOP_ITEMS = {',
...
function patchServerIndexForDevelopmentInvoices() {
  ...
  const replacement = [
    ...
    '',
    '  const expectedItemId = developmentInvoiceItemId(action);',

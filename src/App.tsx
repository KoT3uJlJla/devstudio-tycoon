...
function TopBar({ state, onMomentumOpen }: { state: GameState; onMomentumOpen: () => void }) {
  ...
  const topbarDay = displayGameDay(state.gameDay);
  return (
      ...
      <div className="brand-row compact-brand-row">
        ...
        <div className="topbar-meta">
          ...
          <span className="badge kaboom date-badge compact-date-badge">
            <span>{t('date.dayShort', { day: topbarDay })}</span>
...
function StudioGoals({ state, update, taskOverrides }: { state: GameState; update: (fn: (state: GameState) => GameState) => void; taskOverrides: TaskCatalogOverrides }) {
  ...
  const clearSubscribeUiUnlockTimer = () => {
    if (subscribeUiUnlockTimer.current === null) return;
    window.clearTimeout(subscribeUiUnlockTimer.current);
    subscribeUiUnlockTimer.current = null;
  };

  const scheduleSubscribeUiUnlock = () => {
    clearSubscribeUiUnlockTimer();
    subscribeUiUnlockTimer.current = window.setTimeout(() => {
      subscribeUiUnlockTimer.current = null;
      subscribeClaimInFlight.current = false;
      setSubscribeStatus((status) => (status === 'opening' || status === 'claiming' ? 'retry' : status));
  ...
  const scheduleSubscribeClaimRetry = (eligibleAt: string | null, showWaiting = false, minDelayMs = 0) => {
    ...
    }, Math.max(pendingSubscribeDelayMs(eligibleAt), minDelayMs));
  };
  ...
  const tryClaimSubscribeGoal = async (reason: 'timer' | 'focus' | 'resume' | 'manual') => {
    ...
    const waitMs = pendingSubscribeDelayMs(pending.eligibleAt);
    if (reason !== 'manual' && waitMs > 500) {
  ...
  const startSubscribeGoal = async (goal: StudioGoalModel) => {
    ...
    scheduleSubscribeUiUnlock();
    const fallbackEligibleAt = new Date(Date.now() + SUBSCRIBE_CLAIM_FALLBACK_DELAY_MS).toISOString();
    savePendingSubscribeGoal(goal.id, fallbackEligibleAt);
    scheduleSubscribeClaimRetry(fallbackEligibleAt, true);
    ...
    <section className="panel daily-tasks comic-card">
      ...
      {visibleGoals.map((goal) => {
        ...
        const buttonText = isSubscribeGoal
          ? (claimed ? '✅' : subscribePending ? t('goals.subscribeHatchMind.checking') : subscribeStatus === 'retry' ? t('common.retry') : goal.buttonLabel ?? t('goals.subscribeHatchMind.button'))

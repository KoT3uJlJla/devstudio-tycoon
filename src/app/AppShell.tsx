...
import { money, scoreDelta, signedPercent, signedScore } from '../domain/formatting';
  ...
  return (
    <main className="app-shell">
      ...
      <ScreenRouter
        screen={state.screen}
        routes={{
          studio: <StudioScreen state={state} onNewProject={startNewProject} update={update} taskOverrides={taskOverrides} onOfficeOpen={() => setStudioOfficeOpen(true)} />,
          develop: <DevelopScreen state={state} update={update} />,
          hire: <HireScreen state={state} update={update} />,
          research: <ResearchScreen state={state} update={update} />,
          shop: <ShopScreen state={state} update={update} onRenameStudio={() => setStudioNamingMode('rename')} />,
          menu: <RatingScreen state={state} update={update} />,
        }}
      />
...
function AudiencePanel({ state, update }: { state: GameState; update: (fn: (state: GameState) => GameState) => void }) {
  ...
  return (
    <section className="audience-card comic-card">
      <div className="section-head compact">
        <div><p className="eyebrow">Интересы аудитории</p><h3>Интересы аудитории</h3></div>

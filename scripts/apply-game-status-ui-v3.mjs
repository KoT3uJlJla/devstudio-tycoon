...
function must(source, needle, label) {
  if (!source.includes(needle)) throw new Error(`apply-game-status-ui-v3 failed: ${label}`);
}
...
patchFile('src/storage.ts', (source) => {
  ...
  if (!next.includes('__devstudioGameClosed')) {
    ...
    next = next.replace(/function isPlainObject\(value: unknown\): value is Record<string, unknown> \{[\s\S]*?\n\}\n/, (match) => match + helper);
  }
...
console.log('apply-game-status-ui-v3: ok');

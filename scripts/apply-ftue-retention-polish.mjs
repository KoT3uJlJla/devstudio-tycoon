...
function insertBefore(source, needle, addition, label) {
  ...
  if (index === -1) throw new Error(`ftue-retention-polish: missing ${label}`);
  return source.slice(0, index) + addition + source.slice(index);
}

function replaceOnce(source, from, to, label) {
  if (!source.includes(from)) throw new Error(`ftue-retention-polish: missing ${label}`);
...
patchFile('src/App.tsx', (source) => {
  ...
  if (!next.includes('FIRST_SESSION_CONTRACT_ID')) {
    next = insertBefore(next, 'export default function App()', appHelpersBlock, 'App helper insertion');
  }
...
console.log('apply-ftue-retention-polish: ok');
